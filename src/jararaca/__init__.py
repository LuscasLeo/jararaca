...
_dynamic_imports: "dict[str, tuple[str, str, str | None]]" = {
    ...
    "get_root_span": (__SPEC_PARENT__, "observability.hooks", None),
    "set_root_span_attribute": (__SPEC_PARENT__, "observability.hooks", None),
    "set_root_span_attributes": (__SPEC_PARENT__, "observability.hooks", None),
...
def __getattr__(attr_name: str) -> object:
    ...
    if module_name == "__module__":
        ...
        # Warm up the other exports that live in the same module so their first access
        # skips this function. Names the module does not define are skipped instead of
        # raising: the warm up is an optimisation, and a stale registry entry has to
        # surface when that name is requested, not when an unrelated one is.
        for sibling, (_, sibling_module, sibling_realname) in _dynamic_imports.items():
            if sibling_module != module_name:
                continue

            sibling_attr = sibling if sibling_realname is None else sibling_realname

            if hasattr(module, sibling_attr):
                g[sibling] = getattr(module, sibling_attr)
